...
-export([http_date/1, ensure_parsed_request_path/1, parse_request_path/1,
         parse_query_string/1, split_path/1, ensure_parsed_cookie/1, parse_cookie/1,
         cookie_header/2, cookie_header/3, encrypt/2, decrypt/2, validate/2,
         format_validate_error/1, parse_content_disposition/1, content_disposition/2, app_dir/1,
         priv_dir/1, priv_dir/2, dispatch_on/2, dispatch_app/2, chain_apps/2, encode_url/2,
         decode_url_part/1]).
...
decrypt(Data, Key) ->
    ...
    {_, UnPadded} = crypto:crypto_one_time(aes_cbc128, PaddedKey, ?NULL_IV_128, Data, false),
    UnPadded.

encrypt(Data, Key) ->
    PaddedData = pad(Data, 16),
    PaddedKey = pad(Key, 16),
    crypto:crypto_one_time(aes_cbc128, PaddedKey, ?NULL_IV_128, PaddedData, true).
...
validate(DataIn, [{Field, Checks} | Rest], DataOut) ->
    ...
    handle_apply_checks(apply_checks(Value, Checks, Field, DataIn), DataIn, Rest, DataOut);
validate(_DataIn, [], DataOut) ->
    {ok, DataOut}.

apply_checks(Value, [Check | Rest], Field, Data) ->
    handle_apply_check(apply_check(Check, Value, Data), Check, Value, Rest, Field, Data);
...
maybe_add_validated({_, undefined}, Data) ->
    Data;
maybe_add_validated(Validated, Data) ->
    [Validated | Data].
...
format_validate_error({Field, {min_length, MinLength}}) ->
    io_lib:format("~s must be at least ~b characters long", [Field, MinLength]);
format_validate_error({Field, NumType})
    when NumType == integer; NumType == float; NumType == number ->
...
create_middleware_app(Create, Upstream) ->
    Create(Upstream).
...
uri_part_encode(B) when is_binary(B) ->
    uri_string:quote(binary_to_list(B)).

%%%===================================================================
%%% Decode
%%%===================================================================

decode_url_part(Part) ->
    uri_string:unquote(Part).
